...
def run_bm25(datasets):
    print("<주의> 시간 조금 걸림.... 10-15분? <주의>")
    mecab = Mecab()
    with open("../input/data/data/wikipedia_documents.json", encoding="utf-8") as json_file:
        json_data = json.load(json_file)
    
    wiki = [json_data[k]["text"] for k in tqdm(json_data.keys())]
    wiki_data = [tokenize(json_data[k]["text"]) for k in tqdm(json_data.keys())]
    
    bm25 = BM25Okapi(wiki_data)
    total = []
    for i in tqdm(range(datasets["validation"].num_rows)):
        tmp = {}
...
def run_sparse_retrieval(datasets, training_args):
    #### retreival process ####
    retriever = SparseRetrieval(tokenize_fn=my_tokenize,
                                data_path="../input/data/data",
                                context_path="wikipedia_documents.json")
    retriever.get_sparse_embedding(False)
