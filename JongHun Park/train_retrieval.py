...
from retrieval_dataset import TrainRetrievalDataset, TestRetrievalDataset
# In[2]:
...
def one_step_train(args, model_checkpoint, batch_list, p_encoder, q_encoder, criterion, scaler):
    if model_checkpoint == 'xlm-roberta-large':
        p_input_ids = batch_list[0]
        p_attention_mask = batch_list[1]
        q_input_ids = batch_list[2]
        q_attention_mask = batch_list[3]
        targets_batch = batch_list[4]
    else:
        p_input_ids = batch_list[0]
        p_attention_mask = batch_list[1]
...
def training(args, model_checkpoint, epoch, train_dataloader, p_encoder, q_encoder, criterion, scaler, optimizer, scheduler, logger):
    ## train
...
def validation(args, model_checkpoint, epoch, valid_dataloader, p_encoder, q_encoder, logger, best_acc, run_name):
    ## valid
...
def train(args, model_checkpoint, p_encoder, q_encoder, train_dataloader, valid_dataloader, criterion, scaler, optimizer, scheduler, logger, run_name):
    # Start training!
    best_acc = 0.0
    ...
    for epoch in train_iterator:
        ...
        p_encoder, q_encoder, scaler, optimizer, scheduler = training(args, model_checkpoint, epoch, train_dataloader, p_encoder, q_encoder, criterion, scaler, optimizer, scheduler, logger)
        best_acc = validation(args, model_checkpoint, epoch, valid_dataloader, p_encoder, q_encoder, logger, best_acc, run_name)
...
def main(args):
    ...
    training_dataset = get_pickle(f"/opt/ml/input/retrieval_dataset/Top{args.top_k}_preprocess_train.pkl")
    validation_dataset = get_pickle(f"/opt/ml/input/retrieval_dataset/Top{args.top_k}_preprocess_valid.pkl")

    train_dataset = TrainRetrievalDataset(args.model_checkpoint, training_dataset, p_tokenizer, q_tokenizer)
    valid_dataset = TestRetrievalDataset(args.model_checkpoint, validation_dataset, p_tokenizer, q_tokenizer)

    p_encoder = Encoder(args.model_checkpoint)
    if args.model_checkpoint != 'xlm-roberta-large':
    ...
    if args.model_checkpoint == 'xlm-roberta-large':
        p_encoder, _ = train(training_args, args.model_checkpoint, p_encoder, p_encoder, train_dataloader, valid_dataloader, criterion, scaler, optimizer, scheduler, logger, args.run_name)
    else:
        p_encoder, q_encoder = train(training_args, args.model_checkpoint, p_encoder, q_encoder, train_dataloader, valid_dataloader, criterion, scaler, optimizer, scheduler, logger, args.run_name)
    print('compete!!')
    
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    
    parser.add_argument('--output_dir', type=str, default='/opt/ml/retrieval_output/')
    ...
    parser.add_argument('--run_name', type=str, default='dense_retrieval')
